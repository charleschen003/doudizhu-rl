...
from server.rule_utils.card import Card, action_space, CardGroup, augment_action_space_onehot60, augment_action_space, clamp_action_idx
from server.rule_utils.utils import get_mask_onehot60
import numpy as np
...
class Decomposer:
    ...
    def get_combinations(self, curr_cards_char, last_cards_char):
        if len(curr_cards_char) > 10:
            ...
            combs = env.get_combinations_nosplit(mask, card_mask)
            ...
            combs = env.get_combinations_recursive(mask[valid, :], cards_target)
